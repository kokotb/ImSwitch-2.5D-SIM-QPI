...
Provides an adapter class around the controller interface DLL in Python.
...
class DllAdapter:
	...
	def DoCommand(self, command):
		if command.startswith('stage.position.command.get'):
			ans = [(0, 100000000), 0]
		if command.startswith('stage.position.command.set'):
			ans = [('value',command), 0]
		return ans
