...
class SLM25DController(ImConWidgetController):
    ...
    def moveMask(self, direction):
        ...
        self._master.SLM25DManager.moveMask(mask, direction, amount)
        image = self._master.SLM25DManager.update(maskChange=True, aberChange=True, tiltChange=True)
    ...
    def saveParams(self):
        ...
        slm_info_dict = self.getInfoDict(self._widget.slmParameterTree.p,
                                         ...
                                         self._master.SLM25DManager.getCenters())
    ...
    def loadParams(self):
        ...
        self._master.SLM25DManager.setGeneral(state_general)
        self._master.SLM25DManager.setCenters(state_pos)
        self._master.SLM25DManager.setAberrationFactors(state_aber)
        self._master.SLM25DManager.saveState(state_general, state_pos, state_aber)
        image = self._master.SLM25DManager.update(maskChange=True, tiltChange=True, aberChange=True)
        self.updateDisplayImage(image)
    ...
    def setMask(self, maskMode):
        ...
        self._master.SLM25DManager.setMask(mask, maskMode)
        image = self._master.SLM25DManager.update(maskChange=True, tiltChange=True, aberChange=True)
        self.updateDisplayImage(image)
    ...
    def applyParams(self):
        ...
        image = self._master.SLM25DManager.update(maskChange=True, tiltChange=True, aberChange=True)
        self.updateDisplayImage(image)
        self._master.SLM25DManager.saveState(state_general=slm_info_dict["general"],
                                          state_aber=slm_info_dict["aber"])

    def applyGeneral(self, info_dict):
        self._master.SLM25DManager.setGeneral(info_dict)

    def applyAberrations(self, info_dict, mask=None):
        self._master.SLM25DManager.setAberrations(info_dict, mask)
