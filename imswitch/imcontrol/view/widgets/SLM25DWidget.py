...
#Signals to control enabling buttons/SLM/displaying preview window
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.refIndexOfTheCoverGlass = QtWidgets.QDoubleSpinBox()
        self.refIndexOfTheCoverGlass.setRange(1., 2.)
        self.refIndexOfTheCoverGlass.setSingleStep(0.01)
        self.refIndexOfTheCoverGlass.setDecimals(2)
        self.topLayout.addWidget(self.refIndexOfTheCoverGlass, 3, 5)
    ...
    def toggle25D(self, bool25D): #Only purpose is to disable/enable 2.5D Start button as SIM is turned on/off.
        if bool25D:
            self.start25D.setEnabled(not bool25D)
            self.stop25D.setEnabled(bool25D)
        else:
            self.start25D.setEnabled(bool25D)
